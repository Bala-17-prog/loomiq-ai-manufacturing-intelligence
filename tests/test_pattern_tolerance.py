...
def test_pattern_tolerance():
    ...
    with open(img_path_plain, 'rb') as f:
        res_plain = client.post('/api/quality/inspect', files={'file': ('anomaly_01.jpg', f, 'image/jpeg')}).json()
    ...
    with open(r'd:\loomiq-ai-manufacturing-intelligence\data\fabric_images\normal\normal_01.jpg', 'rb') as f:
        res_normal = client.post('/api/quality/inspect', files={'file': ('normal_01.jpg', f, 'image/jpeg')}).json()
    ...
    with open(img_path_pattern, 'rb') as f:
        res_pattern = client.post('/api/quality/inspect', files={'file': ('texture_01.jpg', f, 'image/jpeg')}).json()
